...
function mostraMensagens(){
		$con = conectaDB();
		$result = mysqli_query($con,"SELECT nome,titulo,mensagem,usuarios.cod FROM usuarios,forum WHERE usuarios.cod  =  forum.codUsuario ORDER BY nome"); 		
		mostraTabela(3,$result,'Mensagem');
		$con->close();
}
	...
	if(@$_REQUEST['action'] == "recuperaRemetentes") //recupera lista de nomes dos remetentes
	{
